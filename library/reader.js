...
function buildRelatedFor(target, targetModule, galleryItems, libraryItems) {
  ...
  function add(module, mid) {
    var key = module + ':' + mid;
    if (seen[key]) return;
    if (module === targetModule && mid === target.id) return;
    seen[key] = true;
    out.push({ module: module, id: mid });
  }
  ...
  function scan(list, module) {
    (list || []).forEach(function(it) {
      (it.relates_to || []).forEach(function(r) {
        if (r && r.module === targetModule && parseInt(r.id, 10) === target.id) add(module, it.id);
      });
    });
  }
...
// Render "Related works" links in the reader info panel.
function renderReaderRelated(related) {
  var el = document.getElementById('reader-related');
  if (!el) return;
  el.innerHTML = '';
  ...
  label.textContent = 'Related works';
  ...
  el.style.display = rendered ? '' : 'none';
}
