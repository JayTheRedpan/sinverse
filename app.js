...
// -- Boot ----------------------------------------
async function init() {
  // Show age gate, load library behind it
  screenAgegate.style.display = 'flex';
  loadLibrary();

  // Button hides the gate and shows the library
...
async function loadLibrary() {
  try {
    ...
    storyGrid.innerHTML = `<div class="loading-placeholder" style="color:#a04040">
      Failed to load story library.<br><small>${err.message}</small>
    </div>`;
  }
}
...
async function fetchNodeCount(meta) {
  try {
    const res = await fetch(`stories/${meta.id}.json`);
    if (!res.ok) return;
...
function renderLibrary() {
  ...
  if (!state.manifest.length) {
    storyGrid.innerHTML = '<div class="loading-placeholder">No stories found.</div>';
    return;
  }

  state.manifest.forEach(meta => {
    const coverSrc   = `images/${meta.id}/cover.jpg`;
    ...
    img.onerror = () => {
      if (!img.src.endsWith('.png')) {
        img.src = `images/${meta.id}/cover.png`;
      } else {
  ...
  newCard.className = 'story-card new-story-card';
  newCard.innerHTML = `
    <div class="new-story-glyph">*</div>
    <h2 class="card-title new-story-title">Start Something New</h2>
    <p class="card-description">Have a story idea set in the Sinverse universe  Find out how to pitch a new story and become a founding author of a fresh branch.</p>
    <div class="card-footer">
      <span></span>
      <span class="card-cta">Submit an idea  </span>
    </div>
  `;
  newCard.addEventListener('click', () => { window.showNewStoryForm(); });
  newCard.addEventListener('keydown', e => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); window.showNewStoryForm(); }
  });
  newCard.tabIndex = 0;
  newCard.setAttribute('role', 'button');
  newCard.setAttribute('aria-label', 'Submit a new story idea');
...
function applyFilters() {
  ...
  const cards      = storyGrid.querySelectorAll('.story-card:not(.new-story-card)');
  ...
  if (!anyVisible && !existing) {
    ...
    msg.textContent = 'No stories match your search.';
...
async function loadStory(id) {
  try {
    ...
    if (!res.ok) throw new Error(`Could not load stories/${id}.json`);
    const data = await res.json();
    ...
    showToast(`Error loading story: ${err.message}`, 4000);
  }
}
...
function renderDeadEnd(node) {
  ...
  deadEndContact.innerHTML = `
    <p class="dead-end-body">
      ...
      submit a continuation -- your scene will be reviewed and added to the story.
    </p>
...
function updateImage(node) {
  if (node.imageId) {
    const base = `images/${state.storyId}/${node.imageId}`;
    sceneImage.alt = '';
    sceneImage.classList.remove('loaded');
    sceneImagePlaceholder.classList.remove('hidden');
    sceneImage.onload  = () => {
      sceneImage.alt = node.imageId.replace(/_/g, ' ');
      sceneImage.classList.add('loaded');
      sceneImagePlaceholder.classList.add('hidden');
    };
    sceneImage.onerror = () => {
      if (!sceneImage.src.endsWith('.png')) {
        sceneImage.src = base + '.png';
      } else {
        sceneImage.classList.remove('loaded');
        sceneImagePlaceholder.classList.remove('hidden');
      }
    };
    sceneImage.src = base + '.jpg';
  } else {
...
btnRestart.addEventListener('click', restartStory);
