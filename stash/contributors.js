...
function isAnon(val) {
  // A collab (array) is "anonymous" only if it has no named credit at all.
  if (Array.isArray(val)) return val.filter(function(n){ return n && !isAnon(n); }).length === 0;
...
// link from the stash).
// Does an item credit this contributor? `field` may be a string or an array
// (collab) — a collab work counts toward EACH listed contributor. Case-sensitive,
// matching the contributor-id convention used everywhere else.
function creditedBy(field, id) {
  var list = Array.isArray(field) ? field : (field != null ? [field] : []);
